 ...
 * L → T, T → R, R → B, B → L
 */
export function rotateTile(tile: number): number {
 ...
 */
export function rotateTileNTimes(tile: number, n: number): number {
  let rotated = tile;
  for (let i = 0; i < (n % 4); i++) {
 ...
 */
export function createTile(sides: string): number {
  let tile = 0;
 ...
 */
export function getTileSides(tile: number): string {
  const sides: string[] = [];
